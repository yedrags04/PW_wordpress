...
namespace WPDataAccess\API {
    ...
    class WPDA_QB extends WPDA_API_Core {
        ...
        public function run( $request ) {
            ...
            return $this->WPDA_Rest_Response(
                '',
                $qb->execute_query( $dbs, $query, $limit, $protect )
            );
        ...
        public function save( $request ) {
            ...
            if ( 'user' === $access ) {
                ...
                if ( 0 === count( $saved )) {
                    $qb->update_query( $dbs, $name, $query, $old_name, $vqb );
                }
            } else {
                if ( $insert ) {
                    // Check if query name is already used
                ...
                if ( 0 === count( $saved )) {
                    $qb->update_query_global($dbs, $name, $query, $old_name, $vqb);
                }
            }
            ...
            if ( null === $enable ) {
                ...
                return $this->WPDA_Rest_Response(
                    get_user_meta(
                        ...
                        self::QUERY_BUILDER_AUTO_COMPLETE,
                        true
                    )
                );
            } elseif ( '1' == $enable ) {
